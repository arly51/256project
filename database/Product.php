...
if ($_POST["action"] == "shwAllPro") {
    ...
    $result = $conn->query("SELECT p.p_id,p.p_title,p.p_subtitle,p.cat_id,p.scat_id,p.p_prize ,p.p_image ,ps.qty as qty ,p.action FROM `product` as p LEFT JOIN pro_stock as ps on p.p_id = ps.pro_id GROUP BY p_id LIMIT {$offset} , {$limit_per_page}");
